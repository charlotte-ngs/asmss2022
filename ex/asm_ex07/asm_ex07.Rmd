...
```{r}
compute_cp_value <- function(n_res_sd_full_model, pn_nr_obs, plm_cur_model){
  n_rssq <- crossprod(residuals(plm_cur_model))
  # model size is the number of predictors plus the intercept
  vec_pred_cur <- attr(terms(plm_cur_model), "term.labels")
  n_model_size <- length(vec_pred_cur) + 1 
  n_cp_cur <- n_rssq / (n_res_sd_full_model^2) - pn_nr_obs + 2 * n_model_size
  return(n_cp_cur)
}
```
...
```{r}
compute_cp_value(n_res_sd_full_model = n_sd_full, pn_nr_obs = nrow(tbl_ex07p01), plm_cur_model = lm_ex07p01_first_subm)
```
