...
Because in an animal model all animals in the pedigree will get a predicted breeding value, we write the vector of breeding values as $\matbf{u}$ and the complete animal model can be written as follows
...
All other components have the same meaning as in the sire model. For the vector $\matbf{u}$, we also have to define the expeted value ($E(\matbf{u})$) and the variance-covariance matrix $var(\matbf{u})$. By their nature, breeding values are defined as deviations from a population mean. This leads to the expected value of $\matbf{u}$ to be $E(\matbf{u}) = \mathbf{0}$. 
