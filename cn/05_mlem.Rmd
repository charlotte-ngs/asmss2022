...
The objective is to predict breeding values for sires $1$, $3$ and $4$ based on the above dataset. The trait pre-weaning gain (`WWG`) is taken as response and `Sex` is assumed to be the only fixed effect. The following values for the variance components are assumed: $\sigma_s^2 = 5$ and $\sigma_e^2 = 55$. 
...
```{r}
library(pedigreemm)
ped_sire <- pedigree(sire = c(rep(NA,2), 1), dam = rep(NA,3), label = as.character(c(1,3,4)))
