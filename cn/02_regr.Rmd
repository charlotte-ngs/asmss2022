...
Figure \@ref(fig:fig-non-linear-rel) shows two examples of non-linear functions. Both examples show that the relationship between the shown variables $x$ and $y$ is not the same over the shown range of values.


## Regression Model
With the regression model we want to find a mathematical way to describe the relationship between the two variables from  our example, breast circumference and body weight. As already mentioned, we are assuming a linear relationship between the two variables. 
